/*
  # Optimize News Headlines Schema

  1. Changes
    - Add optimized indexes for recent article queries
...
-- Create optimized index for recent articles query
...
DO $$ BEGIN
  -- Check if valid_neutralization_status constraint exists
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'valid_neutralization_status'
  ) THEN
